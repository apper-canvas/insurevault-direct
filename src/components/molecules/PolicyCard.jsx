            ...
            <div
                ...
                <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="font-semibold text-gray-900">{policy.asset?.name || policy.assetName || `${policy.type || "Asset"} Insurance`}</h3>
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-semibold text-gray-900">₹{policy.premium.toLocaleString()}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Coverage</span>
                            <span className="text-sm font-medium text-gray-900">₹{policy.coverageAmount.toLocaleString()}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                        ...
                        {policy.ncb > 0 && <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">NCB</span>
                            <Badge variant="success">{policy.ncb}%</Badge>
                        </div>}
                    </div>
                    {isExpiringSoon() && !policy.snoozedUntil && <div
                        className="bg-gradient-to-r from-warning/10 to-yellow-100 border border-warning/20 rounded-lg p-4 mb-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <ApperIcon name="Clock" className="w-5 h-5 text-warning" />
                                <div>
                                    <p className="text-sm text-warning font-semibold">Renewal Reminder
                                                        </p>
                                    <p className="text-xs text-warning/80">Expires in {Math.ceil((new Date(policy.endDate) - new Date()) / (1000 * 60 * 60 * 24))}days
                                                        </p>
                                </div>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <Button variant="success" size="sm" onClick={() => onRenew?.(policy)}>Renew Now
                                                  </Button>
                            </div>
                        </div>
                    </div>}
                    {!isComparison && <div className="flex gap-2">
                        <Button
                            variant="outline"
                            size="sm"
                            className="flex-1"
                            onClick={() => onViewDetails?.(policy)}>View Details
                                      </Button>
                        {policy.status === "active" && <>
                            <Button
                                ...
                                <ApperIcon name="RefreshCw" className="w-4 h-4 mr-2" />Renew
                                              </Button>
                            <Button
                                variant="ghost"
                                size="sm"
                    ...
                    {isComparison && <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="flex justify-between items-center">
                            ...
                            <Button
                                ...
                                {isSelected ? <>
                                    <ApperIcon name="Check" className="w-4 h-4 mr-2" />Selected
                                                    </> : <>
                                    <ApperIcon name="Plus" className="w-4 h-4 mr-2" />Compare
                                                    </>}
                            </Button>
                        </div>
                    </div>}
